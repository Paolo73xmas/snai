...
async def initialize_admin_user():
    ...
    async with db_manager.async_session_maker() as db:
        ...
        # Ensure admin has a user_profile with default password
        await _ensure_admin_profile(db, admin_user_id, admin_user_email)


async def _ensure_admin_profile(db: AsyncSession, admin_user_id: str, admin_email: str):
    """Ensure admin user has a user_profile with production credentials."""
    import hashlib
    from models.user_profiles import User_profiles

    result = await db.execute(
        select(User_profiles).where(User_profiles.user_id == admin_user_id)
    )
    ...
    if profile:
        # Ensure password_hash and username are set to production values
        if not profile.password_hash or profile.username != admin_username:
            profile.password_hash = password_hash
            profile.username = admin_username
            await db.commit()
            logger.info(f"Updated admin profile with production credentials: {admin_username}")
    else:
        # Create admin profile with production credentials
        ...
        db.add(profile)
        await db.commit()
        logger.info(f"Created admin profile with production credentials: {admin_username}")
