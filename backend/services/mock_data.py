...
async def initialize_mock_data():
    ...
    # Ensure admin user always has a password set
    await _ensure_admin_password()


async def _ensure_admin_password():
    """Ensure the production admin user exists in user_profiles with correct credentials."""
    import hashlib
    ...
    admin_username = "admin1@trezzanosnai.it"
    admin_password = "Test123!"
    admin_hash = hashlib.sha256(admin_password.encode()).hexdigest()

    try:
        async with db_manager.engine.begin() as conn:
            # Check if the production admin user already exists
            result = await conn.execute(
                text("SELECT user_id, username, password_hash FROM user_profiles WHERE username = :un"),
                {"un": admin_username}
            )
            ...
            if existing:
                ...
                if existing[2] != admin_hash:
                    await conn.execute(
                        text("UPDATE user_profiles SET password_hash = :ph WHERE username = :un"),
                        {"ph": admin_hash, "un": admin_username}
                    )
                    logger.info(f"Updated password for production admin: {admin_username}")
                else:
                    logger.info(f"Production admin already configured: {admin_username}")
                return

            # Also ensure any admin users without a password get the production credentials
            result = await conn.execute(
                text("SELECT user_id, username FROM user_profiles WHERE ruolo = 'admin' AND (password_hash IS NULL OR password_hash = '')")
            )
            admins_without_pw = result.fetchall()

            for admin_row in admins_without_pw:
                user_id = admin_row[0]
                await conn.execute(
                    text("UPDATE user_profiles SET password_hash = :ph, username = :un WHERE user_id = :uid"),
                    {"ph": admin_hash, "un": admin_username, "uid": user_id}
                )
                logger.info(f"Set production admin credentials for user_id={user_id}, username={admin_username}")
    except Exception as exc:
        logger.error(f"Failed to ensure admin password: {exc}")
