...
async def login_password(
    ...
    # Look up user by username (which could be email)
    from models.user_profiles import User_profiles
    result = await db.execute(
        sa_select(User_profiles).where(
    ...
    if not profile:
        # Also try matching by email field if username didn't match
        result = await db.execute(
            sa_select(User_profiles).where(
                (User_profiles.username == username)
            )
    ...
    # Get password_hash from DB via raw SQL since it may not be in ORM model
    pw_result = await db.execute(
        text("SELECT password_hash FROM user_profiles WHERE id = :pid"),
        {"pid": profile.id}
    )
    stored_hash = pw_result.scalar()
    ...
    user = User(
        ...
        name=f"{profile.nome} {profile.cognome}".strip() or profile.username,
